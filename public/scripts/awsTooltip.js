...
const AWSTooltip = {
  init(canvas, tooltip) {
    ...
    const getServiceDetails = (serviceId, project) => {
      const service = AWSArchitecture.architectures[project]?.services.find(s => s.id === serviceId);
      if (!service || !serviceDetails[serviceId]) return null;
    ...
    const updateTooltip = (e) => {
      ...
      const x = (e.clientX - rect.left - AWSArchitecture.offsetX) / AWSArchitecture.scale;
      const y = (e.clientY - rect.top - AWSArchitecture.offsetY) / AWSArchitecture.scale;
      let foundService = null;
      const { services } = AWSArchitecture.architectures[AWSArchitecture.currentProject];
      foundService = services.find(service => Math.hypot(x - service.x, y - service.y) < 30);

      if (foundService && serviceDetails[foundService.id]) {
        const tooltipHTML = getServiceDetails(foundService.id, AWSArchitecture.currentProject);
        if (tooltipHTML) {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    AWSTooltip.init(document.getElementById('aws-canvas'), document.getElementById('aws-tooltip'));
  });
} else {
  AWSArchitecture.init('aws-canvas', 'aws-tooltip');
  AWSTooltip.init(document.getElementById('aws-canvas'), document.getElementById('aws-tooltip'));
