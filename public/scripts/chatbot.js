...
function updateButtonStates() {
  ...
  document.querySelector('.chat-input-area button').disabled = isLoading;
  document.querySelectorAll('.suggestion-btn').forEach(function(btn) { btn.disabled = isLoading; });
  const micBtn = document.querySelector('.mic-btn');
  if (micBtn) micBtn.disabled = isLoading || !recognition;
}
...
function toggleRecording() {
  ...
  const micBtn = document.querySelector('.mic-btn');
  micBtn.textContent = isRecording ? '🎤 Stop' : '🎤 Speak';
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  micButton.setAttribute('aria-label', 'Toggle speech input');
