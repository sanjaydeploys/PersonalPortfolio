(function(){
  ...
  function initializeNav() {
    ...
    // Close all submenus at all depths
    function closeAllSubmenus(parent = navMenu) {
      ...
      toggles.forEach(toggle => {
        ...
        if (submenu) {
          ...
          closeAllSubmenus(submenu); // Recursively close nested submenus
        }
      });
    }
    ...
    function toggleSubmenu(toggleItem, submenu) {
      ...
      if (!isExpanded) {
        ...
        submenu.querySelectorAll('.nav-sub-link').forEach(slnk => slnk.classList.remove('anim-in'));
        closeAllSubmenus(submenu); // Close nested submenus
      }
      // Adjust parent menu height if on mobile
      if (window.innerWidth <= 1024 && navMenu) {
        setTimeout(() => {
          navMenu.style.maxHeight = 'none';
        }, 500);
    ...
    navMenu?.addEventListener('click', (e) => {
      ...
      const submenu = toggleItem?.querySelector('.subtree');
      if (!submenu) return;
      ...
      siblingToggles.forEach(sibling => {
        if (sibling !== toggleItem && sibling.getAttribute('aria-expanded') === 'true') {
          ...
          const siblingSubmenu = sibling.querySelector('.subtree');
          if (siblingSubmenu) {
