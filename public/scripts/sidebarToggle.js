(function(){
  console.log('[AdvancedNavbar+Sidebar] Loaded - Awe Level Edition v14');
  ...
  function initializeNav() {
    ...
    // Wait for DSA and Case Studies links to ensure DOM availability
    waitForElement('.nav-dsa-links', (dsaLinks) => {
      waitForElement('.nav-case-studies-links', (caseStudiesLinks) => {
        ...
        dsaToggle?.addEventListener('click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          if (!dsaLinks || !dsaToggleItem) return;
          ...
          if (isExpanded) {
            closeAllSubmenus();
          } else if (caseStudiesLinks && caseStudiesToggleItem) {
            caseStudiesToggleItem.setAttribute('aria-expanded', 'false');
            toggleHeight(caseStudiesLinks, false);
          }
        });
        ...
        caseStudiesToggle?.addEventListener('click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          if (!caseStudiesLinks || !caseStudiesToggleItem) return;
          ...
          if (isExpanded) {
            closeAllSubmenus();
          } else if (dsaLinks && dsaToggleItem) {
            dsaToggleItem.setAttribute('aria-expanded', 'false');
            toggleHeight(dsaLinks, false);
          }
        });
        ...
        submenuToggles.forEach((subToggle, index) => {
          subToggle.addEventListener('click', (e) => {
            ...
            submenuToggleItems.forEach((otherItem, otherIndex) => {
              if (index !== otherIndex && otherItem.nextElementSibling) {
