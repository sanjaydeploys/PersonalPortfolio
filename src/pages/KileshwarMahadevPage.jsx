...
const KileshwarMahadevPage = memo(() => {
  ...
  const [loading, setLoading] = useState(!window.__kileshwar_mahadev_DATA__);
  ...
  useEffect(() => {
    ...
    const scripts = [
      ...
      { src: '/public/scripts/languageToggle.js', defer: true },
    ...
    // Handle SSR data
    if (window.__kileshwar_mahadev_DATA__) {
      setSsrHtml(document.documentElement.outerHTML);
      setLoading(false);
    } else {
      fetch('https://gj48940cgb.execute-api.ap-south-1.amazonaws.com/prod/kileshwar-mahadev-ji-neemuch')
        .then((res) => {
        ...
        .catch((error) => {
          console.error('[KileshwarMahadevPage.jsx] Error fetching SSR HTML:', error);
          setLoading(false);
        });
    }
  ...
  if (loading) {
    return (
      <LoadingContainer>
        ...
        <LoadingText>Loading Kileshwar Mahadev page...</LoadingText>
      </LoadingContainer>
    );
  }
