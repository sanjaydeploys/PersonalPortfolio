...
const KileshwarMahadevPage = memo(() => {
  ...
  const [loading, setLoading] = useState(!window.__lic_neemuch_srs_DATA__);
  ...
  useEffect(() => {
    ...
    // Handle SSR data
    if (window.__lic_neemuch_srs_DATA__) {
      setSsrHtml(document.documentElement.outerHTML);
      setLoading(false);
    } else {
      fetch('https://gj48940cgb.execute-api.ap-south-1.amazonaws.com/prod/lic-neemuch-srs')
        .then((res) => {
