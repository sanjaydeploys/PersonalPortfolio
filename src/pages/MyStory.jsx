...
const KileshwarMahadevPage = memo(() => {
  ...
  const [loading, setLoading] = useState(!window.__sanjay_story_DATA__);
  ...
  useEffect(() => {
    ...
    // Handle SSR data
    if (window.__sanjay_story_DATA__) {
      setSsrHtml(document.documentElement.outerHTML);
      setLoading(false);
    } else {
      fetch('https://gj48940cgb.execute-api.ap-south-1.amazonaws.com/prod/story')
        .then((res) => {
